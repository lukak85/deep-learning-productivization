...
def main(host, port, sequence="../../testing/test.mp4"):
    url = f"{host}:{port}"
    ...
    with grpcclient.InferenceServerClient(url=url) as triton_client:
        ...
        for preprocessed_img in preprocessed_frame_list:
            value_data = np.full(shape=preprocessed_img.shape, fill_value=preprocessed_img, dtype=np.float32)

            inputs = []
            inputs.append(grpcclient.InferInput("images", [*preprocessed_img.shape], "FP32"))
            ...
            triton_client.async_stream_infer(
                ...
                sequence_end=(count == len(preprocessed_frame_list)),
            )
    ...
    for results in fp_result_list:
        ...
        if out is None:
            out = cv2.VideoWriter('result_stream.avi', cv2.VideoWriter_fourcc(*'XVID'), 30, (frame_list[0].shape[1], frame_list[0].shape[0]))
